...
are `file:line` at aiopquic 0.3.7-wip. Companion: aiomoqt sits above
...
|---|---|---|---|
| `tx_event_ring` | asyncio → worker | commands: MARK_ACTIVE, FC credit, WT control. Fixed ~64 B entries, no stream payload (pull model) | 2048 entries (`callback.h:62`) |
| `rx_event_ring` | worker → asyncio | notifications: data-ready, FIN, RESET, DESTROY, drain wakes. Payload-less for stream data | 16384 entries (`callback.h:73`) |
| `sc->tx` | asyncio → worker | one per stream: outbound payload bytes | 4 MiB (`callback.h:46`); raw QUIC overridable via `stream_ring_cap` |
...
push path; **WT sends wake unconditionally** — `_transport.pyx:2723`),
and `rx_notify_pending` dedupes eventfd writes (`callback.h:546-558`).

WebTransport shares everything above: `wt_session->bridge` *is* the
transport's `aiopquic_ctx_t` — same event rings, same eventfd
(`h3wt_callback.h:101,147`). WT differs only in callbacks and
...
sequenceDiagram
    ...
    participant SCTX as sc->tx (per stream)
    participant WK as picoquic worker
    participant NIC as UDP / NIC

    Note over App,API: NEW STREAM only — gates before creation
    alt aggregate queued > tx_max_queued_bytes (8 MiB)
        API->>API: park 2 ms poll until < cap/2<br/>(raw: send_stream_data_drained new-stream branch;<br/>WT: create_stream → _await_tx_queued_capacity)
    end
    ...
    alt this stream's sc->tx used > tx_max_inflight_bytes (2 MiB, aiomoqt)
        API->>API: park on per-stream drain event<br/>until < cap/2 (clear→arm→recheck→wait)
    end
    API->>CY: send_stream_data
    CY->>CY: pre-check tx_event_ring slot
    CY->>SCTX: commit bytes (all-or-nothing)
    CY->>TXR: TX_MARK_ACTIVE
    CY->>WK: wake (raw: coalesced via tx_wake_pending)
    API-->>App: return (no copy retained)

    Note over WK,NIC: worker pull, wire pace
    WK->>WK: pop MARK_ACTIVE → picoquic_mark_active_stream
    WK->>SCTX: prepare_to_send / provide_data:<br/>pop min(used, budget), set fin/still_active
    ...
    alt sc->tx was full (BufferError raised to caller)
        WK-->>API: STREAM_TX_DRAINED (CAS-armed, once per fill/drain;<br/>re-armed if rx_event_ring full)
        API->>API: asyncio.wait FIRST_COMPLETED<br/>{per-stream event, ring event} → retry same buffer
    end
    alt tx_event_ring ≤ 50% after being full
...
|---|---|---|---|---|
| 1 | `tx_max_queued_bytes` (8 MiB, `configuration.py:110`) | aggregate `pushed − pulled − discarded` over cap | park at stream creation; resume < cap/2; 2 ms poll | all streams — the only bound short-stream churn cannot bypass |
| 2 | `tx_max_inflight_bytes` (2 MiB, aiomoqt `protocol.py:66`) | one stream's `sc->tx` used over cap | park mid-stream on drain event; resume < cap/2 | per stream — fairness + binds on long-lived streams |
...
`tx_data_bytes_pushed/pulled/discarded` (`stream_ctx.h:163-192`);
...
sequenceDiagram
    participant NIC as UDP / NIC
    participant WK as picoquic worker
    participant SCRX as sc->rx (per stream)
    ...
    WK->>SCRX: push bytes (ring = advertised window;<br/>overflow ⇒ cnx close, INTERNAL_ERROR on wire)
    alt no data-ready notification in flight (CAS rx_event_pending 0→1)
        WK->>RXR: STREAM_DATA notification (payload-less)
        WK->>EV: notify (coalesced)
    else one already pending
        WK->>WK: skip — rx_data_event_coalesced++<br/>(≤ 1 outstanding per stream, by construction)
    end
    Note over RXR: FIN / RESET / DESTROY always push

    EV->>AIO: reader callback
    Note over AIO: raw client → drain_rx_callback;<br/>server engine + WT dispatcher → drain_rx
    AIO->>AIO: clear rx_event_pending BEFORE pop
    AIO->>SCRX: pop ALL available bytes → malloc StreamChunk
    AIO->>AIO: rx_consumed += n; pending_release += n (chunk wrap)
    AIO->>API: StreamDataReceived(memoryview)
    Note over API: next_event: deque, drains ring ONLY when<br/>local queue empty — ingest bounded to one batch
    ...
    AIO->>AIO: per drain cycle: dedupe one credit push per stream;<br/>hysteresis: only if rx_consumed advanced ≥ ring/16
    AIO->>WK: OPEN_FLOW_CONTROL event (via tx_event_ring)
    WK->>WK: grant = sc->rx free − pending_release
    WK->>NIC: MAX_STREAM_DATA (+ MAX_DATA bump) → peer
```
...
- **Replenishment is exclusively consumer-driven.** The worker grants
  ...
  (`_transport.pyx:1587-1597`). There is no worker-side extension
  loop.
...
- **`pending_release` throttles only untouched bytes.** It is
  ...
  (`_transport.pyx:673`) — once a memoryview exists, bounding memory
...
- **Consumer-lag stall chain**: consumer busy → `next_event` skips
  ...
  *on that stream*. Nothing blocks the asyncio loop; data events stay
  ...
  entries remain (`callback.h:583-591`), so no lost wakeups.
- **MAX_DATA (connection level) is picoquic-automatic**: extended at
  frame *receipt* when `2 × offset_received > maxdata_local`
  (`sender.c:2924-2936`) — it never reflects application backpressure.
...
Full dict: `TransportContext.counters` (`_transport.pyx:940-1025`);
